setwd("D:/REVA UNIVERSITY - PG/PAPER")
...
mod3<-lm(formula = AmountSpent ~  Location + Salary + Catalogs + Children1+Med_d+Low_d , data = data1)
...
actuals_preds <- data.frame(cbind(actual, predicted))  # make actuals_predicteds dataframe.
correlation_accuracy <- cor(actuals_preds)  # 90.9%
correlation_accuracy 
head(actuals_preds)

#create run chart
p<-ggplot(data,aes(x=row(data)[,2],y=predicted))
p+geom_line(colour="blue")+geom_line(data=data,aes(y=actual),colour="black")
...
####
actuals_preds <- data.frame(cbind(actual, predicted))  # make actuals_predicteds dataframe.
correlation_accuracy <- cor(actuals_preds)  # 93%
head(actuals_preds)
